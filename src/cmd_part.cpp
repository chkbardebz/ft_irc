...
// PART #<channel1>, #<channel2> :<msg d'adieu>
...
//! RAJOUTER FEATURE FERMETURE CHANNEL SI < 0 CLIENT
bool part(std::map<int, Client> &huntrill, int client_fd, char* line, Server &serverDetails)
{
    ...
    if (!(ss >> cmd >> channel_s_unsplited)) //? PART est forcement la car parsing mais channel a echoue donc err
     ...
     channels_splited_it != channels_splited.end(); ++channels_splited_it) 
    {
        ...
        // std::cout << "debug=" << *channels_splited_it << std::endl;  
        if ((*channels_splited_it)[0] != '#')
        {
        ...
        {
            if (send_msg_to_channel(serverDetails, huntrill, client_fd, *channels_splited_it, message) == false)
                continue ;
        } //!
        else //!
        { //!
            if (channel_details->second.is_fd_in_channel(client_fd) == false)
                return (write(client_fd, "403 ERR_NOSUCHCHANNEL\n", 23), clear_channels_splited(channels_splited), false);       
            channel_details->second.client_quit_channel(client_fd);
        }
        if (channel_details->second.channel_size() == 0) //? fermeture du channel si aucun client
            serverDetails.makala.erase(channel_details);
    }
    clear_channels_splited(channels_splited);
    return (true);
}


    // if (channel_s[0] != '#')
    //     return (write(client_fd, "476 ERR_BADCHANMASK", 20), false); //? tous les possibles channels doivent etre essayer mm si le premier contient une erreur



//     ERR_NOTONCHANNEL ==> quand le client tente de quitter un channel dont il n'est pas membre
//?    403 ERR_NOSUCHCHANNEL ==> gerer dans send_msg_to_channel
//?    ERR_NEEDMOREPARAMS ==> quand PART seul
